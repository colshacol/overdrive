import mysql from "../mysql"

export default async (app) => {
  app.post("/api/v0/associateParcelWithProject", async (request, response) => {
    const [sql, pool, procedures] = await mysql
    const invokeProcedure = procedures.associateParcelWithProject(request.body)

    const handleSuccess = (data) => {
      return response.send({ isSuccess: true, data })
    }

    const handleError = (error) => {
      console.log("[ERROR] /api/v0/associateParcelWithProject", error)
      return response.status(400).send({ isSuccess: false, error })
    }

    invokeProcedure.then(handleSuccess)
    invokeProcedure.catch(handleError)
  })
}
